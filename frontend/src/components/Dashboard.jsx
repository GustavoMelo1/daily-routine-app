    ...
    <div>
      <div className="flex flex-col gap-4 border-b border-zinc-200 pb-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold capitalize tracking-tight text-zinc-950">
      ...
      {error ? (
        ...
        <>
          ...
          <section className="mt-6 overflow-hidden rounded-xl border border-zinc-200 bg-white" aria-labelledby="activity-title">
            <div className="border-b border-zinc-200 px-5 py-4">
              <h2 id="activity-title" className="font-semibold text-zinc-950">Atividade diária</h2>
              <p className="mt-1 text-sm text-zinc-500">Estudo e execução classificados separadamente.</p>
            </div>

            {loading ? (
              <div className="space-y-3 p-5" aria-label="Carregando atividade">
                {[1, 2, 3].map((item) => <div key={item} className="h-12 animate-pulse rounded-lg bg-zinc-100" />)}
              </div>
            ) : sortedDays.length === 0 ? (
              <p className="p-8 text-center text-sm text-zinc-500">Nenhum dia registrado neste período.</p>
            ) : (
                ...
                {sortedDays.map((day) => {
                  ...
                  return (
                    <div key={day.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[110px_1fr_1fr_auto] sm:items-center">
                      <span className="text-sm font-semibold capitalize text-zinc-800">{formatCompactDate(day.data)}</span>
                      <span className="text-sm text-zinc-600">Estudo: {day.minutos_estudados} min</span>
                      <span className="text-sm text-zinc-600">Tarefas: {completed}/{day.tarefas.length}</span>
                      <span className={`w-fit rounded-full px-2.5 py-1 text-xs font-semibold ${
                        qualifies ? "bg-blue-50 text-blue-700" : "bg-zinc-100 text-zinc-500"
                      }`}>
